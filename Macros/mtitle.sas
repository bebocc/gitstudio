...
data toc_prog;
  ...
  call symput('title1',trim(title1)||repeat(" ",(&mls-length(title1)-length(tabnum)-30-4+offset))||"[Page_00000_of_99999]");
  ...
  %IF &TYPE=LST or &TYPE=PS %THEN %DO;
    ...
    call symput('ltitle2',"&ptitle2" || repeat(" ",(&ls-length("&ptitle2")-30-1))); ** || "[Page_00000_of_99999]");
  ...
  else if index(tabnum,'Appendix') then call symput('outputf','Listings');
...
%else %do;
  ...
  title4 j=l "&tabnum:  &title1";

  %do i=2 %to &numtitle;

    title%eval(&i+3) j=c "&&title&i" ;
